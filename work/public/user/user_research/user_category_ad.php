...
<?php
$ca_ad=$_GET['name'];
$ca_ad=htmlspecialchars("$ca_ad", ENT_QUOTES, 'UTF-8');
...
<div class="lbn_main">
	<div class="fav_lbn">
    <h2><?= ($ca_ad);?>カテゴリーの検索結果</h2>
        <div class="showlbn1">
            <?php foreach($disp_data as $ca_ad): ?>
                <?php if ($ca_lbn['soldout'] == '1') :?>
                    <a href ="../user_lbn_details/user_lbn_details.php?lbn_id=<?="{$ca_ad['lbn_id']}"; ?>">             
                        <div class="sold_out_ust-show">
                        <div class="sold_out_border">
                            <img src ="<?= "{$ca_ad['file_path']}"; ?>" ,alt= "" class="sold_out_ust-showpic">
                        </div>
                        </div>
                </div>
                    </a>
                <?php else: ?>
                        <a href ="../user_lbn_details/user_lbn_details.php?lbn_id=<?="{$ca_ad['lbn_id']}"; ?>">
                            <div class="ust-show">
                                <img src ="<?= "{$ca_ad['file_path']}"; ?>" ,alt= "" class="ust-showpic">
                            </div>
                        </a>
                <?php endif ?>
            <?php endforeach?>
    ...
    <div>
            <p class="uca_all">全件数&ensp;:&ensp;<?=$files_num ?>件</p>
        </div>
