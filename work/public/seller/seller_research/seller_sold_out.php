...
$sold_out=htmlspecialchars("$sold_out", ENT_QUOTES, 'UTF-8');

define('MAX','20');
$files = GetSoldOut($sold_out);
...
?>
            ...
            <span class="st-header-title">販売済の投稿<span>
                <div class="st-showlbn">
                    <?php foreach($disp_data as $sold_out): ?>
				            <a href ="../seller_lbn_details/seller_lbn_details.php?lbn_id=<?="{$sold_out['lbn_id']}"; ?>">
					<div class="st-show">
                        <form action="sold_out_check.php" method="post"  class="sold_out_button">
                            <input type=hidden name="lbn_id" value="<?= $sold_out['lbn_id']?>">
                    ...
                    <img src ="<?= "{$sold_out['file_path']}"; ?>" ,alt= "" class="st-showpic">
                        <?php if ($sold_out['soldout'] == '1') :?>
                            <div class="st_sold_out">
                                <span class="st_sold_out_text">売約済</span>    
                            </div>
                            <?php endif ?>
                    <div class="st-price-content">
                        <p class="st-price">¥<?=$sold_out['price']?></p>
                    </div>
				</div>
				</a>
			<?php endforeach?>
                ...
                for($i = 1; $i <= $max_page; $i++){ // 最大ページ数分リンクを作成
                    if ($i == $now) { // 現在表示中のページ数の場合はリンクを貼らない
                        ...
                        echo '<a href="seller_lbn_all.php?.php?page_id='.$i.'" class="sla_a">'. $i .'</a>';
                    }
                }
